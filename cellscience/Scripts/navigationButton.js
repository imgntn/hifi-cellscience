(function() {
	...
	this.preload = function(entityId) {
		...
		print("BODY PITCH: " + JSON.stringify(MyAvatar.bodyPitch) + "BODY YAW: " + JSON.stringify(MyAvatar.bodyYaw) + "BODY ROLL: " + JSON.stringify(MyAvatar.bodyRoll));
	}
	...
	this.addButton = function() {
		...
		print('NAVIGATION BUTTON OVERLAY IS::' + this.button)
	...
	this.onClick = function(event) {
		...
		if (clickedOverlay == self.button) {
			print("Clicked navigation button: " + self.userData.name + ", and looking at " + self.userData.target.x + ", " + self.userData.target.y + ", " + self.userData.target.z);
	...
	this.unload = function() {
		print('NAVIGATION BUTTON UNLOAD!!!', this.button, this.entityId)
		Overlays.deleteOverlay(self.button);
		Controller.mousePressEvent.disconnect(self.onClick);
	}

	Controller.mousePressEvent.connect(self.onClick);
